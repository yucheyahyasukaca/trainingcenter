...
export default function EditContentPage({ 
  ...
  const [contentData, setContentData] = useState<LearningContent | null>(null)
  const [loading, setLoading] = useState(true)
  const [hasAccess, setHasAccess] = useState(false)
  const [saving, setSaving] = useState(false)
  const [contents, setContents] = useState<LearningContent[]>([])
  
  const [editedContent, setEditedContent] = useState<Partial<LearningContent>>({
    title: '',
    description: '',
    content_type: 'text',
    content_data: {},
  ...
  const calculateProgress = () => {
    ...
    steps.basicInfo = !!(editedContent.title?.trim())

    // Check Tipe Konten (Content Type)
    steps.contentType = !!(editedContent.content_type)

    // Check Konten Materi (Content Data) - based on content type
    if (editedContent.content_type === 'text') {
      steps.contentData = !!(editedContent.content_data?.body?.trim())
    } else if (editedContent.content_type === 'video') {
      steps.contentData = !!(editedContent.content_data?.video_url?.trim())
    } else if (editedContent.content_type === 'document') {
      steps.contentData = !!(editedContent.content_data?.document_url?.trim())
    } else if (editedContent.content_type === 'quiz') {
      steps.contentData = !!(editedContent.content_data?.question?.trim())
    } else if (editedContent.content_type === 'assignment') {
      steps.contentData = !!(editedContent.content_data?.instructions?.trim())
    }

    // Check Pengaturan (Settings) - validate required settings
    steps.settings = !!(editedContent.estimated_duration && editedContent.status)
  ...
  }, [params.id, params.classId, params.contentId, profile, authLoading])
  ...
  async function handleSave() {
    ...
    try {
      ...
      addToast.error('Gagal memperbarui materi')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <nav className="flex items-center gap-1 text-xs sm:hidden">
            ...
            <span className="text-slate-400">/</span>
            <span className="text-slate-700 font-medium">Edit Materi</span>
          </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6 lg:p-8">
              ...
              <div className="relative">
                <select
                  value={editedContent.content_type || 'text'}
                  onChange={(e) => setEditedContent({ ...editedContent, content_type: e.target.value as any })}
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6 lg:p-8">
              ...
              {editedContent.content_type === 'text' && (
                <div>
                  <RichTextEditor
                    value={editedContent.content_data?.body || ''}
                    onChange={(value) => setEditedContent({
                      ...editedContent,
                      content_data: { ...editedContent.content_data, body: value }
                    })}
              ...
              {editedContent.content_type === 'video' && (
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="relative">
                      <input
                        type="url"
                        value={editedContent.content_data?.video_url || ''}
                        onChange={(e) => setEditedContent({
                          ...editedContent,
                          content_data: { ...editedContent.content_data, video_url: e.target.value }
                        })}
                  ...
                  <div>
                    ...
                    <textarea
                      value={editedContent.content_data?.transcript || ''}
                      onChange={(e) => setEditedContent({
                        ...editedContent,
                        content_data: { ...editedContent.content_data, transcript: e.target.value }
                      })}
                      rows={6}
              ...
              {editedContent.content_type === 'document' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-3">
                      URL Dokumen
                    ...
                    <div className="relative">
                      <input
                        type="url"
                        value={editedContent.content_data?.document_url || ''}
                        onChange={(e) => setEditedContent({
                          ...editedContent,
                          content_data: { ...editedContent.content_data, document_url: e.target.value }
                        })}
                        className="w-full px-4 py-4 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={editedContent.content_data?.document_name || ''}
                      onChange={(e) => setEditedContent({
                        ...editedContent,
                        content_data: { ...editedContent.content_data, document_name: e.target.value }
                      })}
                      className="w-full px-4 py-4 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
              ...
              {editedContent.content_type === 'quiz' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-3">
                      Pertanyaan Quiz
                    ...
                    <textarea
                      value={editedContent.content_data?.question || ''}
                      onChange={(e) => setEditedContent({
                        ...editedContent,
                        content_data: { ...editedContent.content_data, question: e.target.value }
                      })}
                      rows={4}
                      className="w-full px-4 py-4 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 resize-none"
              ...
              {editedContent.content_type === 'assignment' && (
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-3">
                      Instruksi Tugas
                    ...
                    <textarea
                      value={editedContent.content_data?.instructions || ''}
                      onChange={(e) => setEditedContent({
                        ...editedContent,
                        content_data: { ...editedContent.content_data, instructions: e.target.value }
                      })}
                      rows={8}
                      className="w-full px-4 py-4 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 resize-none"
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={editedContent.content_data?.due_date || ''}
                      onChange={(e) => setEditedContent({
                        ...editedContent,
                        content_data: { ...editedContent.content_data, due_date: e.target.value }
                      })}
                      className="w-full px-4 py-4 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
                    />
                  </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6 lg:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="number"
                      value={editedContent.estimated_duration || 10}
                      onChange={(e) => setEditedContent({ ...editedContent, estimated_duration: parseInt(e.target.value) || 10 })}
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      value={editedContent.status || 'draft'}
              ...
              <div className="mt-6 sm:mt-8 space-y-3 sm:space-y-4">
                <label className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 border-2 border-slate-200 rounded-xl hover:border-slate-300 transition-all duration-200 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={editedContent.is_free || false}
                    onChange={(e) => setEditedContent({ ...editedContent, is_free: e.target.checked })}
                ...
                <label className="flex items-start gap-3 sm:gap-4 p-3 sm:p-4 border-2 border-slate-200 rounded-xl hover:border-slate-300 transition-all duration-200 cursor-pointer group">
                  <input
                    type="checkbox"
                    checked={editedContent.is_required || false}
                    onChange={(e) => setEditedContent({ ...editedContent, is_required: e.target.checked })}
                    className="mt-1 w-4 h-4 sm:w-5 sm:h-5 text-blue-600 border-2 border-slate-300 rounded focus:ring-blue-500"
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
            ...
            <div className="sticky top-4 sm:top-8 space-y-4 sm:space-y-6">
              ...
              <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-slate-200 p-4 sm:p-6">
                <div className="space-y-2 sm:space-y-3">
                  <button
                    onClick={handleSave}
                    disabled={saving || !editedContent.title?.trim()}
